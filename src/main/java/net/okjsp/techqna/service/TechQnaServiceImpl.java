...
public class TechQnaServiceImpl implements TechQnaService {
	...
	public List<TechQna> selectTechQnaList(Paging paging, String sortType, String tagName) {
		...
		List<TechQna> techQnaList = tagName == null ? 
				techQnaDao.selectTechQnaList(sortType, paging.getOffset(), paging.getSizePerList()) : 
				techQnaDao.selectTechQnaListByTag(tagName, sortType, paging.getOffset(), paging.getSizePerList());
		
		for(TechQna techQna : techQnaList) {
			techQna.setRecommendationList(recommendationService.getRecommendation(2, techQna.getWriteNo()));
			techQna.setTagList(tegService.selectTagList(2, techQna.getWriteNo()));
	...
	public List<TechQna> selectTechQnaDetail(Integer WriteNo) {
		...
		// 상동. (question count + answer count) * 3 의 DB UI가 발생함. association 학습 후 리펙토링!
